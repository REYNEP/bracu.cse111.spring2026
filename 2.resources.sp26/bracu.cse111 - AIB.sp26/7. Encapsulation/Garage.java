class Garage{
  ...
  public void storeCar( Car c ){
    if( count<this.cars.length ){
      this.cars[count] = c;
      count++;
      System.out.println(c.getModel()+" has been parked in "+this.name+"'s Garage");
    } else {
  ...
  public void showCars( String color ){
    for( int i=0; i<this.cars.length; i++ ){
      Car c = this.cars[i];
      if( c!=null && c.getColor()==color ) c.printDetails();
    }
  }
  public void removeCar(){
    if( count > 0 ){
      ...
      System.out.println(c.getModel()+" has been removed!");
    } else {
